...
public:
	...
	//0 simlarest to previous, 1 y-top, 2 same as diffrp
	void get_next_camera_pos(Eigen::Matrix4d now_camera_pose_world, Eigen::Vector3d object_center_world, int type_of_pose = 2) {
		switch (type_of_pose) {
			case 0:
			{
				//归一化乘法
...
public:
	...
	void get_view_space(vector<Eigen::Vector3d>& points) {
		...
		for (int i = 0; i < share_data->pt_sphere.size(); i++) {
			...
			double scale = 1.0 / share_data->pt_norm * share_data->view_space_radius;
